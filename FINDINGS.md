...
## 13. `build.py`'s freshness stamp describes a file it never re-checks — OURS

**Not the plug. This is `build.py`, and it made the fixed point vacuous for one
run without saying anything.**

`assemble()` is guarded by `fresh(WASM, [WAT], force)`, which compares the sha of
the INPUT (`generated/codexwasm.wat`) against `generated/local/codexwasm.wasm.fp`.
It never asks whether `generated/codexwasm.wasm` is still the file that stamp was
written for. `generated/local/` is gitignored and `generated/` is tracked, so a
`git checkout -- generated/` restores the artifact and leaves the stamp behind,
describing a module that is no longer there.

Measured 2026-09-01:

    generated/local/codexwasm.wasm.fp   b7fd495b...   (the sha of the NEW wat)
    generated/codexwasm.wasm            4ef33bdb, 806060 bytes  -- the OLD module

Generation 1 wrote the new WAT, whose sha then matched the stamp, so stage 3
printed `codexwasm.wasm is current (806060 bytes) -- not reassembling` and
skipped. **Generation 2 therefore ran the SAME module generation 1 had run**, and

    IDENTICAL -- 6508722 bytes, both ways

is a statement about determinism, not about the fixed point. The invariant this
repository exists for -- *the module compiles the source that produced it* -- was
not tested, and the verdict said `fixed point HOLDS`.

`--force` reassembles (806061 bytes, sha 4dde1d14), generation 2 runs that module,
and the fixed point holds for real. That is how the artifacts beside this file
were produced.

**The fix is to key the stamp on the OUTPUT as well as the input**, or to verify
the output's sha before trusting the stamp -- the same correction
`harness/wasm_plug_build.py` needed in safari-codex (its finding 12) and the same
shape as its finding 14. Three instances in one day of a freshness check that
cannot see the thing that actually moved; this one is the worst of them, because
the check it silenced is this project's only invariant.

**A `--prove-gate` run would NOT have caught it.** That perturbs generation 2's
bytes and requires the comparison to go red, which it would -- the comparison
works. What failed is that both sides came from one module.
